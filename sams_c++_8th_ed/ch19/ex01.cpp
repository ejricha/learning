...
class ContactItem {
 public:
  ...
  bool operator==(const ContactItem& item_to_compare) const {
    ...
    //cout << "this->phone_number = (" << this->phone_number << "), item_to_compare.phone_number = (" << item_to_compare.phone_number << ")\n";
    return (this->phone_number == item_to_compare.phone_number);
  }
...
int main() {
  ...
  cout << "Enter a phone number you wish to delete: ";
  string input_phone_number;
  getline(cin, input_phone_number);

  // Find the contact in the set
  auto contact_found = set_contacts.find(ContactItem("", input_phone_number));
  if (contact_found != set_contacts.end()) {
    set_contacts.erase(contact_found);
    cout << "Displaying contacts after erasing " << input_phone_number << "\n";
    display(set_contacts);
  }
  else {
